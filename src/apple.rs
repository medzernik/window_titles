...
impl ConnectionTrait for Connection {
    ...
    fn window_titles(&self) -> Result<Vec<(u32, String)>> {
        ...
        let command_pid = Command::new("osascript")
            .args(arguments)
            .output()
        ...
        let command_window = Command::new("osascript")
            .args(arguments)
            .output()
            .expect("failed to execute AppleScript command");
        ...
        match (
            error_window.contains(PERMISSION_ERROR),
            error_pid.contains(PERMISSION_ERROR),
        ) {
            (false, false) => Ok(split(
                &String::from_utf8_lossy(&command_window.stdout),
                &String::from_utf8_lossy(&command_pid.stdout),
            )),
